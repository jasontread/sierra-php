...
displStr                                    if this attribute is an entity, 
                                            this parameter defines the parse str 
																						...
																						be an entity or a DateTime object or 
...
pre                                         html/text to display before the attribute value

post                  &nbsp;                html/text to display after the attribute value
...
class                                       an optional class to apply to 
                                            the attribute value in the form of an 
...
indexTpl                          [tpl]     if the attribute index is shown, it may be 
...
indexPos              0           (0|1|2|3) the relative placement for the attr index
                                            See 'labelPos' comnent for more info. 
																						To not show the index, set this 
																						parameter to 0
...
{if $class}<font {if $params->getParam('classTag')}{$params->getParam('classTag')}{else}class{/if}="{cycle name=$cycleName values=$class}">{/if}
{if ($params->getParam('linked') eq '1' && $attribute && (!$params->getParam('displStr') || ($params->getParam('displStr') && $attribute->parseString($params->getParam('displStr')))))}
{assign var="linkCycleName" value=$params->getParam('linkCycle', $params->getParam('linkClass'))}
<a{if $params->getParam('linkClass')}{if $params->getParam('linkClassTag')} {$params->getParam('linkClassTag')}{else} class{/if}="{cycle name=$linkCycleName values=$params->getParam('linkClass')}"{/if} href="{$params->getParam('linkPre', '')}{if $params->getParam('displStr')}{$attribute->parseString($params->getParam('displStr'))}{else}{$attribute}{/if}">
{/if}
{if $params->getParam('isDate')}
	{if $attribute}
		{assign var="tmp" value=$params->getParam('displStr')}
		{assign var="tmp" value=$attribute->format($params->getParam('linkDisplStr', $tmp))}
		{$tmp}
	{/if}
{elseif $params->getParam('displStr')}
	{if $attribute}
		{assign var="tmp" value=$params->getParam('displStr')}
		{assign var="tmp" value=$attribute->parseString($params->getParam('linkDisplStr', $tmp))}
		{$tmp}
	{/if}
{else}
	{if $params->getParam('linkResource') && $attribute}
		{$resources->getString($params->getParam('linkResource'))}
	{else}
		{if $params->getParam('isBool')}
			{if $attribute}
				{$resources->getString($params->getParam('labelTrue', 'form.yes'))}
			{else}
				{$resources->getString($params->getParam('labelFalse', 'form.no'))}
			{/if}
		{else}
			{$resources->getString($attribute)}
		{/if}
	{/if}
{/if}
{if $params->getParam('linked') eq '1' && $attribute && (!$params->getParam('displStr') || ($params->getParam('displStr') && $attribute->parseString($params->getParam('displStr'))))}</a>{/if}
{if $class}</font>{/if}
