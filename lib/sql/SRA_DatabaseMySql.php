...
class SRA_DatabaseMySql extends SRA_Database {
    ...
    function commit($conn=null) {
        ...
        else
        {
            ...
            // in a transaction.
            $result = !function_exists('mysql_pconnect') ? mysqli_query($conn, 'COMMIT') : mysql_query('COMMIT', $conn);

            if ($result === FALSE)
            {
                // SRA_Error.
                $msg = "SRA_DatabaseMySql::commit(): SRA_Error: ". (!function_exists('mysql_pconnect') ? mysqli_error($conn) : mysql_error($conn));
                return(SRA_Error::logError($msg, __FILE__, __LINE__));
            }
        }
    ...
    function &convertBlob(&$blob) {
        ...
        // NOTE: This assumes the  blob or text datatype is being used.
				return "'" . (!function_exists('mysql_pconnect') ? mysqli_escape_string($this->_getAppDbConnection(), $blob) : mysql_escape_string($blob)) . "'";
    }
    // }}}
  ...
  // }}}
    ...
    function convertText($text) {
        ...
        return("'". (!function_exists('mysql_pconnect') ? mysqli_escape_string($this->_getAppDbConnection(), $text) : mysql_escape_string($text)) . "'");
    }
    // }}}
    ...
    function &execute($query, $incCol=FALSE, $errorLevel=SRA_ERROR_PROBLEM, $conn=NULL)
    {
        ...
        else
        {
            ...
            //return new SRA_ExecuteSet(1);
						!function_exists('mysql_pconnect') ? mysqli_select_db($conn, $this->_dbName): mysql_select_db($this->_dbName, $conn);
            $result = !function_exists('mysql_pconnect') ? mysqli_query($conn, $query) : mysql_query($query, $conn);

            if ($result === FALSE)
            {
                // SRA_Error.
                $msg = "SRA_DatabaseMySql::execute(): ".(!function_exists('mysql_pconnect') ? mysqli_error($conn) : mysql_error($conn))." $query";
                // NOTE: This SRA_Error uses $errorLevel.
                return(SRA_Error::logError($msg, __FILE__, __LINE__, $errorLevel));
            }

            // Create an SRA_ExecuteSet.
            $xs = new SRA_ExecuteSet(!function_exists('mysql_pconnect') ? mysqli_affected_rows($conn) : mysql_affected_rows($conn));
            ...
            if ($incCol)
            {
                ...
                $xs->setSequenceValue(!function_exists('mysql_pconnect') ? mysqli_insert_id($conn) : mysql_insert_id($conn));
            }
    ...
    function &fetch($query, $types=NULL, $limit=FALSE, $offset=FALSE, $errorLevel=SRA_ERROR_PROBLEM)
    {
        ...
        if (!$result)
        {
            ...
            if (preg_match('/gone away/', $msg)) {
              ...
              $this->_dbs = NULL;
              $conn = $this->_getAppDbConnection($baseQuery, TRUE);
              !function_exists('mysql_pconnect') ? mysqli_select_db($this->_getAppDbConnection(NULL, TRUE), $this->_dbName) : mysql_select_db($this->_dbName, $this->_getAppDbConnection(NULL, TRUE));
              $result = !function_exists('mysql_pconnect') ? mysqli_query($conn, $query, MYSQLI_USE_RESULT) : mysql_unbuffered_query($query, $conn);
            }
            // NOTE: This SRA_Error uses $errorLevel.
        ...
        // Cast results.
        $num_cols = !function_exists('mysql_pconnect') ? mysqli_num_fields($result) : mysql_num_fields($result);

        while ($row = !function_exists('mysql_pconnect') ? mysqli_fetch_row($result) : mysql_fetch_row($result))
        {
            // Switch on each col.
        ...
        if (!function_exists('mysql_pconnect')) mysqli_free_result($result);
    ...
    function getNextSequence($sequence, $errorLevel=SRA_ERROR_PROBLEM, $conn=null)
    {
        ...
        else
        {
            // Step 3:
            // There's a $conn. Execute query.
            $result = !function_exists('mysql_pconnect') ? mysqli_query($conn, "INSERT INTO $sequence VALUES (0)") : mysql_query("INSERT INTO $sequence VALUES (0)", $conn);

            if (!$result)
            {
                // SRA_Error.
                $msg = "SRA_DatabaseMySql::getNextSequence: Failed - ". (!function_exists('mysql_pconnect') ? mysqli_error($conn) : mysql_error($conn)) .", $sequence";
                // NOTE: This SRA_Error uses $errorLevel.
                return(SRA_Error::logError($msg, __FILE__, __LINE__, $errorLevel));
            }

            return(!function_exists('mysql_pconnect') ? mysqli_insert_id($conn) : mysql_insert_id($conn));
        }
    }
    // }}}
    ...
    function selectDb($newDb) {
      $this->_dbName = $newDb;
      !function_exists('mysql_pconnect') ? mysqli_select_db($this->_getAppDbConnection(), $this->_dbName) : mysql_select_db($this->_dbName, $this->_getAppDbConnection());
      if ($this->_readOnlyDb) {
        !function_exists('mysql_pconnect') ? mysqli_select_db($this->_getAppDbConnection(NULL, TRUE), $this->_dbName) : mysql_select_db($this->_dbName, $this->_getAppDbConnection(NULL, TRUE));
      }
      return TRUE;
    ...
    function _openConn($config)
    {
        ...
        if (!(!function_exists('mysql_pconnect') ? mysqli_select_db($conn, $config['name']) : mysql_select_db($config['name'], $conn)))
        {
            // SRA_Error.
    ...
    function rollback($conn=null)
    {
        ...
        else
        {
            ...
            $result = !function_exists('mysql_pconnect') ? mysqli_query($conn, 'ROLLBACK') : mysql_query('ROLLBACK', $conn);

            if ($result === FALSE)
            {
                // SRA_Error.
                $msg = "SRA_DatabaseMySql::rollback(): SRA_Error: ". (!function_exists('mysql_pconnect') ? mysqli_error($conn) : mysql_error($conn));
                return(SRA_Error::logError($msg, __FILE__, __LINE__));
            }
        }
    }
    ...
    function startTransaction($conn=null)
    {
        ...
        else
        {
            ...
            $result = !function_exists('mysql_pconnect') ? mysqli_query($conn, 'BEGIN') : mysql_query('BEGIN', $conn);

            if ($result === FALSE)
            {
                // SRA_Error.
                $msg = "SRA_DatabaseMySql::startTransaction(): SRA_Error: ". (!function_exists('mysql_pconnect') ? mysqli_error($conn) : mysql_error($conn));
                return(SRA_Error::logError($msg, __FILE__, __LINE__));
            }
        }
    }
