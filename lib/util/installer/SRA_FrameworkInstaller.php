  ...
  function _testDbConnection($config) {
    ...
    switch($config['type']) {
      ...
      case SRA_DB_TYPE_MSSQL:
        ...
        break;
      default:
        ($conn = (!function_exists('mysql_pconnect') ? mysqli_connect($config['host'], $config['user'], $config['password'], NULL, $config['port'] ? $config['port'] : NULL) : mysql_connect($config['host'] . ':' . $config['port'], $config['user'], $config['password']))) ? (!(!function_exists('mysql_pconnect') ? mysqli_select_db($conn, $config['name']) : mysql_select_db($config['name'], $conn)) ? $ret = FALSE : $ret = TRUE) : $ret = FALSE;
        if ($conn) {
