...
{/if}
   ...
   * @param boolean $dirtyOnly whether or not to only copy dirty attributes
   ...
   * @access public
	...
	// {ldelim}{ldelim}{ldelim} copyInto
	/**
	 * copies this objects attributes into $obj
	 * @param object $obj the entity to copy the attributes into
	 * @param boolean $dirtyOnly whether or not to only copy dirty attributes
   * @param boolean $recursive whether or not to perform the copy recursively. 
   * when TRUE, any nested entity attributes will also be copied. otherwise, 
   * the copy will contain references to the same entities
...
{/if}
    foreach(array_keys($initVals) as $key) {ldelim}
      if (!$dirtyOnly || $this->isDirty($key)) {ldelim}
        $result = TRUE;
        $obj->setAttribute($key, $initVals[$key]);
      {rdelim}
    {rdelim}
    return $result;
	{rdelim}
...
{/foreach}
   ...
   * @param boolean $persistentOnly if TRUE, only persistent attribute names will 
   * be returned
	 * @return array
	 * @access public
	 */
	function &getAttributeNames($persistentOnly=FALSE) {ldelim}
	  if (!isset($this->__attrNames)) {ldelim}
      $this->__attrNames = array();
{foreach from=$entity->getAttributes() item=attribute}
{if $attribute->_skipPersistence}
      if (!$persistentOnly) {ldelim}
{/if}
      $this->__attrNames[] = '{$attribute->_name}';
...
{/foreach}
    {rdelim}
    return $this->__attrNames;
	{rdelim}
	// {rdelim}{rdelim}{rdelim}
	
	// {ldelim}{ldelim}{ldelim} getColumnNames
	/**
	 * Returns an array of all of the column names for this entity including 
   * aggregate attributes indexed by attribute name
   * @param string $attr if specified, only the name of the column associated 
   * with this attribute will be returned
	 * @return array
	 * @access public
	 */
	function &getColumnNames($attr=NULL) {ldelim}
	  if (!isset($this->__colNames)) {ldelim}
      $this->__colNames = array();
{foreach from=$entity->getAttributes() item=attribute}
{if !$attribute->_skipPersistence}
      $this->__colNames['{$attribute->_name}'] = '{$attribute->_column}';
{if $attribute->getAggregateAttributes()}
      // aggregate attributes from {$attribute->_name}
{foreach from=$attribute->getAggregateAttributes() item=attr}
      $this->__colNames['{$attr->_name}'] = '{$attr->_column}';
{/foreach}
{/if}
{/if}
{/foreach}
    {rdelim}
    return $attr ? $this->__colNames[$attr] : $this->__colNames;
	{rdelim}
	// {rdelim}{rdelim}{rdelim}
